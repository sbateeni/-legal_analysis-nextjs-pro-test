...
export default function RegisterOffice() {
  ...
  const handleAdminSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1) محاولة تهيئة قاعدة البيانات على الخادم مرة واحدة عبر API آمن
      try {
        await fetch('/api/db-init', { method: 'POST' })
      } catch (e) {
        // تجاهل إن فشلت (قد تكون مُهيأة مسبقاً)
      }

      // 2) التحقق من هيكل قاعدة البيانات بعد محاولة التهيئة
      console.log('🔍 فحص هيكل قاعدة البيانات...')
      const dbStructure = await checkDatabaseStructure()
      
      // 3) إن لم تكتمل الجاهزية، نفذ تهيئة عبر Supabase rpc كخطة بديلة
      if (!dbStructure.hasOfficeIdColumns || !dbStructure.hasOfficesTable) {
        console.log('🚀 قاعدة البيانات تحتاج تهيئة (خطة بديلة)...')
